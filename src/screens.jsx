...
/* ══════════════════════════════════════════════════════════════════
   WATCH: daily goal ring
══════════════════════════════════════════════════════════════════ */
export function WatchFace({ pct = 73, size = 150, band = '#2c2c30' }) {
  const ref = useRef(null)
  ...
  useEffect(() => {
    if (!inView) return
    let n = 0
    const iv = setInterval(() => { n += 2; setV(Math.min(n, pct)); if (n >= pct) clearInterval(iv) }, 22)
    return () => clearInterval(iv)
  }, [inView, pct])

  const w = size, h = size * 1.22
  return (
    <div ref={ref} style={{ width:w,height:h,position:'relative',flexShrink:0 }}>
      {/* Band */}
      <div style={{ position:'absolute',left:'50%',top:-size*0.22,transform:'translateX(-50%)',
        width:w*0.52,height:size*0.3,background:band,borderRadius:'8px 8px 3px 3px' }} />
      <div style={{ position:'absolute',left:'50%',bottom:-size*0.22,transform:'translateX(-50%)',
        width:w*0.52,height:size*0.3,background:band,borderRadius:'3px 3px 8px 8px' }} />
      {/* Case */}
      <div style={{ position:'absolute',inset:0,borderRadius:size*0.27,background:'#1a1a1e',
        boxShadow:`0 22px 50px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.1)`,padding:size*0.045 }}>
        <div style={{ width:'100%',height:'100%',borderRadius:size*0.23,background:C.navy,
          position:'relative',overflow:'hidden',display:'flex',flexDirection:'column',
          alignItems:'center',justifyContent:'center',gap:2 }}>
          <Grain op={0.1} z={1} />
          <img src="/logos/Jeani J White.png" alt="" style={{ height:size*0.085,opacity:0.9,position:'relative',zIndex:2 }} />
          <div style={{ fontFamily:F.body,fontSize:size*0.055,letterSpacing:1.4,
            color:'rgba(255,255,255,0.5)',position:'relative',zIndex:2 }}>DAILY GOAL</div>
          <div style={{ fontFamily:F.display,fontSize:size*0.29,fontWeight:700,color:'#fff',
            lineHeight:1,position:'relative',zIndex:2 }}>{v}%</div>
        </div>
      </div>
      {/* Crown */}
      <div style={{ position:'absolute',right:-3,top:'36%',width:4,height:size*0.11,
        background:'#3a3a40',borderRadius:'0 2px 2px 0' }} />
    </div>
  )
}
